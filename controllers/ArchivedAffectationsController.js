...
const afficherArchivedAffectation = async (req, res) => {
  try {
    // Récupérer tous les collaborateurs et véhicules
    const collaborateurs = await Collaborateur.find();
    const vehicules = await Vehicule.find();

    res.render('archivedaffectations', { collaborateurs, vehicules });
  } catch (err) {
    console.error(err);
...
const NouvelleArchivedAffectation = async (req, res) => {
  try {
...
const getArchivedAffectationDetails = async (req, res) => {
  try {
    const archivedaffectationDetails = await ArchivedAffectationModel.aggregate(
      [
        {
          $lookup: {
            from: 'collaborateurs',
    ...
    console.log('Archived affectation details:', archivedaffectationDetails);
    return res.send(archivedaffectationDetails);
  } catch (error) {
    console.error(error);
    return res
      .status(500)
